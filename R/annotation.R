...
annotateSpatialData <- function(
  ...
  if (label %in% colnames(metadata)) {
    unique_names <- make.unique(c(
      colnames(metadata)[grepl(paste0("^", label), colnames(metadata))],
      label
    ))
  ...
  if (!is.null(annotation_assay)) {
    layer_metadata <- sample_metadata[
      sample_metadata$Layer == sample_metadata[assay, "Layer"] &
        sample_metadata$Sample == sample_metadata[assay, "Sample"],
    ]
    if (annotation_assay %in% layer_metadata$Assay) {
      # get segments
      segments <- vrSegments(object, assay = annotation_assay)
      segments <- lapply(segments, function(seg) {
        seg[, colnames(seg)[colnames(seg) != "id"]]
      })
      segment_names <- gsub("_Assay[0-9]+$", "", names(segments))

      # remove the latest annotation
      all_assay_names <- vrAssayNames(object, assay = "all")
      object <- subsetVoltRon(
        object,
        assays = all_assay_names[
          !all_assay_names %in%
            rownames(layer_metadata)[layer_metadata$Assay == annotation_assay]
        ]
      )
    } else {
      segments <- list()
    ...
    sidebarLayout(
      ...
      sidebarPanel(
        ...
        p(
          ...
          strong("Single-L-click"),
          " to select polygon or circle points"
        ),
        p(
          style = "font-size: 12px;",
          strong("Add Region"),
          " to set points as a new region"
        ),
        p(
          style = "font-size: 12px;",
          strong("Circles"),
          " require only 2 points"
        ),
        p(
          style = "font-size: 12px;",
          strong("Polygons"),
  ...
  server <- function(input, output, session) {
    ...
    output$textbox_ui <- renderUI({
      lapply(rev(textboxes()), function(i) {
        column(
          12,
          textInputwithButton(
            textinputId = paste0("region", i),
            label = paste0("Region ", i),
            buttoninputId = paste0("remove", i),
            value = isolate(textbox_values[[paste0("region", i)]]),
    ...
    observe({
      lapply(textboxes(), function(i) {
        observeEvent(
          input[[paste0("region", i)]],
          {
            textbox_values[[paste0("region", i)]] <- isolate(input[[paste0(
              "region",
              i
            )]])
          },
    ...
    observeEvent(input$addregion_btn, {
      ...
      if (isolate(input$region_type == "Polygon")) {
        if (nrow(selected_corners()) > 3) {
          ...
          showNotification(
            "You must selected at least 4 points for each polygon!"
          )
        }
      }
    ...
    output$image_plot <- renderPlot({
      ...
      if (!use.image.only) {
        ...
        g <- g +
          transformSpatialLayer(g_spatial_clone, img, ranges, max.pixel.size)
      }

      # visualize currently selected corners ####
      transformed_corners <- transformSelectedCorners(
        selected_corners(),
        img,
        ranges,
        max.pixel.size
      ...
      transformed_corners_list <- lapply(
        selected_corners_list(),
        transformSelectedCorners,
        img,
        ranges,
        max.pixel.size
      )
      if (length(transformed_corners_list) > 0) {
        for (i in seq_len(length(transformed_corners_list))) {
          ...
          # visualize labels
          if (ncol(cur_corners) == 2) {
            cur_corners <- data.frame(
              x = mean(cur_corners[, 1]),
              y = max(cur_corners[, 2]),
              region = paste("Region ", isolate(textboxes()[i]))
            )
          } else {
            cur_corners <- data.frame(
              x = cur_corners[, 1],
              y = cur_corners[, 2] + cur_corners[, 3],
              region = paste("Region ", isolate(textboxes()[i]))
            )
          }
          g <- g +
            ggrepel::geom_label_repel(
              mapping = aes(x = x, y = y, label = region),
              data = cur_corners,
              size = input$label_size,
              direction = "y",
              nudge_y = 6,
              box.padding = 0,
              label.padding = input$label_size * 0.1,
              seed = 1,
              color = "red"
            )
        }
    ...
    observeEvent(input$done, {
      ...
      # collect labels
      selected_label_list <- vapply(
        seq_len(length(selected_polygon_list)),
      ...
      if (length(selected_corners_list()) == 0) {
        ...
        for (i in seq_len(length(selected_label_list))) {
          segments[[selected_label_list[i]]] <- data.frame(
            id = i,
...
manageImageBrush <- function(
  ...
  observeEvent(input$plot_dblclick, {
    ...
    if (!is.null(brush)) {
      ...
      # if width is large, then correct the brush 
      # event for the downsize (scaling) effect
...
manageSelectedCorners <- function(
  selected_corners,
  image,
  ranges,
  max.pixel.size,
...
transformSelectedCorners <- function(
  selected_corners,
  image,
  ranges,
  max.pixel.size
  ...
  selected_corners <- selected_corners -
    matrix(
      rep(limits[1, ], nrow(selected_corners)),
      nrow = nrow(selected_corners),
      byrow = T
    )
...
transformSpatialLayer <- function(g_spatial, image, ranges, max.pixel.size) {
  # correct for zoom
  ind <- (g_spatial$data$x > ranges$x[1] & g_spatial$data$x < ranges$x[2]) &
    (g_spatial$data$y > ranges$y[1] & g_spatial$data$y < ranges$y[2])
...
textInputwithButton <- function(
  ...
  div(
    ...
    shiny::tags$button(
      ...
      `data-val` = buttonvalue,
      disabled = NULL,
      list(shiny::icon("trash")),
      ...
    )
  )
}
...
makeCircleData <- function(selected_corners) {
  radius <- sqrt(
    (selected_corners[1, 1] - selected_corners[2, 1])^2 +
      (selected_corners[1, 2] - selected_corners[2, 2])^2
  )
  data.frame(
    x = selected_corners[1, 1],
...
#' point.in.circle
#'
#' checks if points are in the defined circle ROI with a (x,y) 
#' center and radius
#'
#' @keywords internal
