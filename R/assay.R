  ...
  if (!is.null(features)) {
    ...
    if (!is.null(spatialpoints)) {
      ...
      for (embed in vrEmbeddingNames(object)) {
        ...
        vrEmbeddings(object, type = embed) <- embedding[
          spatialpoints[spatialpoints %in% rownames(embedding)],
          ,
          drop = FALSE
        ]
      }
      ...
      for (img in vrSpatialNames(object)) {
        object@image[[img]] <- subsetvrImage(
          object@image[[img]],
      ...
      for (embed in vrEmbeddingNames(object)) {
        embedding <- vrEmbeddings(object, type = embed)
        vrEmbeddings(object, type = embed) <- embedding[
          rownames(embedding) %in% spatialpoints,
          ,
          drop = FALSE
        ]
      }
...
subsetCoordinates <- function(coords, imageinfo, crop_info) {
  ...
  if (inherits(coords, "IterableMatrix")) {
    ...
    inside <- (coords[, 1] > xlim[1] & coords[, 1] < xlim[2]) &
...
subsetSegments <- function(segments, imageinfo, crop_info) {
  ...
  # subset
  cropped_segments <- subsetCoordinates(
    segments[, c("x", "y")],
    imageinfo,
    crop_info
  )
  if (any(colnames(segments) %in% c("rx", "ry"))) {
    cropped_segments_extra <- segments[
      rownames(cropped_segments),
      c("rx", "ry")
    ]
    cropped_segments <- cbind(cropped_segments, cropped_segments_extra)
  }
  cropped_segments <- data.frame(
    cropped_segments,
    id = segments[rownames(cropped_segments), 1],
    row_id = rownames(cropped_segments)
  )
  cropped_segments <- cropped_segments %>%
    right_join(
      segments[, c(colnames(segments)[1], "row_id")],
      by = c("row_id" = "row_id")
    )
  if (any(colnames(segments) %in% c("rx", "ry"))) {
    cropped_segments <- cropped_segments[, c(
      colnames(cropped_segments)[which(grepl(
        colnames(segments)[1],
        colnames(cropped_segments)
      ))[1]],
      "x",
      "y",
      "rx",
  ...
  if (value %in% names(object@data)) {
    ...
    if (ignore) {
      warning(
        ...
        "' is not found in '",
        vrAssayNames(object),
        "'. Main feature type is still set to '",
...
setMethod(
  "vrMainFeatureType<-",
  "vrAssay",
  function(object, ignore = FALSE, value) {
...
addFeaturevrAssayV2 <- function(object, data, feature_name) {
  ...
  if (length(setdiff(colnames(data), vrSpatialPoints(object))) > 0) {
    stop(
      "The number of spatial points is not matching with number of points in the input data"
    )
  }
  ...
  if (length(embed_names) > 0) {
    for (type in embed_names) {
      if (nrow(embeddings[[type]]) > 0) {
        rownames(embeddings[[type]]) <- value[match(
          rownames(embeddings[[type]]),
          spatialpoints
        )]
...
vrAssayNamesReplacevrAssay <- function(object, value) {
  ...
  # change assay names
  spatialpoints <- stringr::str_replace(
  ...
  if (vrAssayTypes(object) %in% c("ROI", "cell", "spot")) {
    ...
    spatialpoints[ind] <- stringr::str_replace(
      spatialpoints[ind],
      "$",
      paste0("_", value)
    )
...
generateTileDatavrAssay <- function(
  ...
  if (vrAssayTypes(object) != "tile") {
    ...
    # make tile data
    image_data <- .make_tiles_data(
      image_data,
      tile_size = vrAssayParams(object, param = "tile.size")
    )
