...
fixVoltRon <- function(object) {
  ...
  for (samp in unique(sample.metadata$Sample)) {
    ...
    if (
      methods::is(catch_connect, 'try-error') ||
        methods::is(catch_connect, 'error')
    ) {
    ...
    for (lyr in unique(sample.metadata$Layer[sample.metadata$Sample == samp])) {
      ...
      # correct
      catch_connect <- try(
        slot(object_layer, name = "connectivity"),
        silent = TRUE
      )
      if (
        methods::is(catch_connect, 'try-error') ||
  ...
  for (curr_idx in seq_len(length(jsons_vec))) {
    ...
    for (targ in curr_json[["Targets"]]) {
      ...
      for (prb in targ[["Probes"]]) {
        ...
        lookup_df[nrow(lookup_df) + 1, ] <- list(
          curr_RTS_ID,
          curr_targ,
          curr_module,
          curr_code_class,
  ...
  list(
    schema = list(
      "Header" = data.frame(
        ...
        row.names = c("FileVersion", "SoftwareVersion", "Date"),
        stringsAsFactors = FALSE
      ),
      "Scan_Attributes" = data.frame(
        labelDescription = c("The sample ID", "The plate ID", "The well ID"),
        row.names = c("ID", "Plate_ID", "Well"),
        minVersion = numeric_version(c(rep("0.01", 3L))),
        stringsAsFactors = FALSE
      ),
      "NGS_Processing_Attributes" = data.frame(
        labelDescription = c(
          NA_character_,
          NA_integer_,
          NA_integer_,
      ...
      "Code_Summary" = data.frame(
        labelDescription = c(NA_character_, NA_integer_),
...
.dccMetadata[["protocolData"]] <-
  do.call(rbind, unname(head(.dccMetadata[["schema"]], 3L)))[,
    "labelDescription",
    drop = FALSE
  ]

rownames(.dccMetadata[["protocolData"]])[
  rownames(.dccMetadata[["protocolData"]]) == "ID"
] <- "SampleID"


.codeClassMetadata <-
  c(
    "CodeClass,IsControl,Analyte",
...
.codeClassMetadata <-
  utils::read.csv(
    textConnection(paste0(.codeClassMetadata, collapse = "\n")),
...
stopQuietly <- function(...) {
  blankMsg <- sprintf(
    "\r%s\r",
    paste(rep(" ", getOption("width") - 1L), collapse = " ")
  )
...
make_css <- function(..., file = NULL) {
  ...
  for (x in css_defs) {
    ...
    if (length(x[[2]]) != length(x[[3]])) {
      stop(
        "The second and third elements of each list need to have the same length"
      )
    }
  }
  all_css <- vapply(
...
#' columns rather than character columns. Adapted from the \code{fastDummies} 
...
dummy_cols <- function(
  ...
  for (col_name in char_cols) {
    ...
    if (!is.null(split)) {
      unique_vals <- unique(trimws(unlist(strsplit(
        unique_vals,
        split = split
      ))))
    }
    ...
    if (remove_most_frequent_dummy) {
      ...
      vals <- data.frame(
        sort(table(vals), decreasing = TRUE),
        stringsAsFactors = FALSE
      )
    ...
    for (unique_value in unique_vals) {
      ...
      if (!is.null(split)) {
        max_split_length <- max(sapply(
          strsplit(as.character(.data[[col_name]]), split = split),
          length
        ))
        ...
        if (is.na(unique_value)) {
          .data[[paste0(col_name, "_", unique_value)]][which(
            !is.na(.data[[col_name]])
          )] <- 0
        }
      }
  ...
  if (omit_colname_prefix) {
    if (length(select_columns) == 1) {
      new_col_index <- as.logical(rowSums(sapply(unique_vals, function(x) {
        grepl(paste0(select_columns, "_", x), names(.data))
      })))
      names(.data)[new_col_index] <- gsub(
        paste0(select_columns, "_"),
        "",
        names(.data)[new_col_index]
      )
    } else {
      message(
        "Can't omit the colname prefix when recoding more than one column."
      )
...
css <- function(..., collapse_ = "") {
  ...
  names(props)[!is_css_var] <- standardize_property_names(names(props)[
    !is_css_var
  ])
...
new_defaults <- function(value = list()) {
  ...
  append <- function(...) {
    ...
    dots <- sapply(
      names(dots),
      function(x) dots[[x]] <- c(defaults[[x]], dots[[x]]),
      simplify = FALSE
    )
    defaults <<- merge(dots)
    invisible(NULL)
...
#' @param verbose toggle to control if the output is ggproto object 
#' (verbose==FALSE,default) or string of layer call (verbose==TRUE)
#' @param showDefaults toggle to control if the verbose output shows all 
#' the input arguments passed to the proto object (if verbose==FALSE 
#' then ignored)
...
cloneLayer <- function(l, verbose = FALSE, showDefaults = TRUE) {
  ...
  parent.layer <- dplyr::left_join(
    proto_features(l),
    dplyr::filter(geom_opts, !grepl("^stat", !!rlang::sym("fn"))),
    by = c("position", "geom", "stat")
  )
  if (is.na(parent.layer$fn)) {
    parent.layer$fn <- paste0(
      tolower(strsplit(parent.layer$stat, "(?<=Stat)", perl = TRUE)[[1]]),
      collapse = "_"
    )
  }
  layer.names <- c(
  ...
  if (verbose) {
    ...
    if (!showDefaults) {
      ...
      geom_diff <- sapply(names(geom_aes)[-1], function(x) {
        geom_aes[[x]][
          !geom_aes[[x]] %in%
            geom_proto[[x]]
        ]
...
cloneProto <- function(l) {
  ...
  nDF[grep("colour|fill|color", nDF[, 1]), 2] <- paste0(
    "'",
    col2hcl(nDF[grep("colour|fill|color", nDF[, 1]), 2], alpha = NULL),
    "'"
  )

  geom_aes$default <- paste0(apply(nDF, 1, function(x) {
    paste0(x, collapse = "=")
  }))
...
#'
#' @description Options for functions in the ggedit package. When running 
#' R code, the object \code{ggedit_opts}
#' (default options) is not modified by chunk headers (local chunk options are
#' merged with default options), whereas \code{ggedit_opts_current} 
#' (current options) changes with different chunk headers and it always 
#' reflects the options for the current chunk.
#'
#' Normally we set up the global options once in the first code chunk in a
#' document using \code{ggedit_opts$set()}, so that all \emph{latter} chunks 
#' will use these options. Note the global options set in one chunk will not 
#' affect the options in this chunk itself, and that is why we often need to 
#' set global options in a separate chunk.
#'
#' @note \code{ggedit_opts_current} is read-only in the sense that it does 
#'   nothing if you call \code{ggedit_opts_current$set()}; you can only query 
#'   the options via \code{ggedit_opts_current$get()}.
#' @rdname ggeditOpts
#' @noRd
ggedit_opts <- new_defaults(list(
  ...
  themeTips = list(
    ...
    element_text = list(
      family = shiny::HTML(
        '<a href="http://www.cookbook-r.com/Graphs/Fonts/" target="_blank">font family</a>'
      ),
    ...
    justification = list(
      justification = 'anchor point for positioning legend inside plot <br/> "center" or two-element numeric vector'
    ),
    position = list(
      position = 'the position of legends. <br/> "left", "right", "bottom", "top", or two-element numeric vector'
    )
  ),

  ThemeDefaultClass = data.frame(
    item = c(
      "angle",
      "background",
      "caption",
...
col2hcl <- function(colour, h = NULL, c = NULL, l = NULL, alpha = NULL) {
  ...
  if (!is.null(alpha)) {
    rgba_col <- grDevices::rgb(
      rgb_col_with_alpha[1],
      rgb_col_with_alpha[2],
      rgb_col_with_alpha[3],
      alpha = alpha
    )
  } else {
    rgba_col <- grDevices::rgb(
