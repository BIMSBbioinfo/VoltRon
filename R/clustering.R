...
getProfileNeighbors <- function(
  ...
  if (data.type %in% c("raw", "norm")) {
    ...
    if (data.type %in% vrEmbeddingNames(object)) {
      ...
      stop(
        "Please provide a data type from one of three choices: raw, norm and pca"
      )
    }
  }
...
#' if NULL, the default assay will be used, see \link{vrMainAssay}.
#' @param label the name for the newly created clustering column in the 
#' metadata.
...
#' @param dims the number of dimensions extracted from the embedding if 
#' data.type is not NULL
#' @param nclus The number of cluster centers for K-means or hierarchical 
#' clustering.
#' @param distance_measure the distance measure used by hierarchical clustering. 
#' See \code{method} for a list of distance measures in \link{dist}.
#' @param abundance_limit the minimum number of points for a cluster, hence 
#' clusters with abundance lower than this limit will be appointed to other 
#' nearby clusters.
#' @param seed seed.
#'
...
getClusters <- function(
  ...
  if (method == "leiden") {
    ...
    clusters <- list(
      names = names(clusters$cluster),
      membership = clusters$cluster
    )
    ...
    stop(
      "Unrecognized clustering method! Use either 'leiden' for graph ",
      "clustering or 'kmeans' for K-means clustering"
    )
  }

  # correct clustering for low abundant clusters
  clusters <- .correct_low_abundant_clusters(
    object_graph,
    clusters,
    abundance_limit
  )
...
#' @noRd
.correct_low_abundant_clusters <- function(
  object_graph,
  clusters,
  abundance_limit
) {
...
#' @noRd
.check_clustering_params <- function(
  method,
  resolution,
  nclus,
  abundance_limit
