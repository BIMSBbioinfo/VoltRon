...
getDeconvolution <- function(
    ...
    if (length(assay.types) > 1) {
        ...
        if (is.null(reference)) {
            stop("Deconvolution reference was not constructed. Check your sc.object and assay parameters.")
        }
    ...
    if (assay.type == "spot") {
        ...
        if (method == "MuSiC") {
            ...
            if (inherits(sc.object, "SingleCellExperiment")) {
                ...
                if (packageVersion("SeuratObject") >= "5.0.0" && exists("JoinLayers", where = asNamespace("SeuratObject"))) {
                    sc.object <- SeuratObject::JoinLayers(sc.object, assay = sc.assay)
                    sccounts <- Seurat::GetAssayData(sc.object, assay = sc.assay, layer = "counts")
                } else {
...
getDeconSingle <- function(
    ...
    if (length(assay.type) != 1) {
        stop("getDeconSingle requires exactly one assay type. Detected: ", paste(assay.type, collapse = ", "))
    }
...
getRCTD <- function(object, features = NULL, reference, sc.cluster, ...) {
    ...
    if (packageVersion("spacexr") >= "2.0.0") {
        ...
        args <- list(...)
        if (!"doublet_mode" %in% names(args)) args$doublet_mode <- 'full'
        
        myRCTD <- do.call(spacexr::run.RCTD, c(list(myRCTD), args[names(args) %in% names(formals(spacexr::run.RCTD))]))
        ...
        if (exists("createRctd", where = asNamespace("spacexr"))) {
            myRCTD <- spacexr::createRctd(spatialdata, 
                                          reference, 
                                          cell_type_col = sc.cluster)
            
            message("Calculating Cell Type Compositions of spots with RCTD ...")
                    
            suppressMessages(myRCTD <- spacexr::runRctd(myRCTD, rctd_mode = 'full', ...))
            
        } else {
            myRCTD <- spacexr::create.RCTD(spatialdata, reference, cell_type_col = sc.cluster)
            
            message("Calculating Cell Type Compositions of spots with RCTD ...")
            
            suppressMessages(myRCTD <- spacexr::run.RCTD(myRCTD, doublet_mode = 'full', ...))
        }
    ...
    if (nrow(results) != n_target) {
        ...
        if (length(common) == 0 && n_target > 0) {
            ...
            if (length(valid_idx) > 0) {
                ...
                stop("RCTD finished, but spot names do not match the VoltRon object (neither exact nor sanitized matches). Check your sample IDs.")
            }
        } else {
    ...
    if (method == "RCTD") {
        ...
        if (inherits(sc.object, "Seurat")) {
            ...
            if (!sc.cluster %in% colnames(sc.object@meta.data)) {
                stop("The cluster column '", sc.cluster, "' was not found in the Seurat object metadata.")
            }
            ...
            if (!inherits(sccounts, "sparseMatrix")) sccounts <- as(sccounts, "dgCMatrix")
