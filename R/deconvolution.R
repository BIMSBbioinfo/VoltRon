...
#'
#' @noRd
getDeconSingle <- function(object, features = features, reference, method = "RCTD", sc.cluster, ...){
...
#' @noRd
getDeconReferenceSpot <- function(sc.object, sc.assay = NULL, sc.cluster, method){
  ...
  if(method == "RCTD"){
    ...
    if(inherits(sc.object, "Seurat")){
      ...
      if(is.null(sc.assay))
        message("The sc.assay arguement is not provided, using", Seurat::DefaultAssay(sc.object), "instead!")
      tryCatch({
      ...
      if (!requireNamespace('SingleCellExperiment'))
        stop("Please install SingleCellExperiment package for using SCE objects")
      if(!is.null(sc.assay))
        message("The sc.assay arguement is provided by ignored for SCE objects!")
