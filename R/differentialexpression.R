...
#' see \link{SampleMetadata}. if NULL, the default assay will be used, 
#' see \link{vrMainAssay}.
#' @param group.by the categorical variable from metadata to get 
#' differentially expressed features across
#' @param group.base Optional, the base category in \code{group.by} which 
  ...
  if (group.by %in% colnames(metadata)) {
    if (!is.null(group.base)) {
      if (!group.base %in% unique(as.vector(metadata[[group.by]]))) {
        stop(
          "Please specify a group that is included in the group.by column ", 
          "of metadata to define the base group!"
        )
      }
    } else {
...
#' @param group.by the categorical variable from metadata to get differentially 
#' expressed features across
#' @param group.base Optional, the base category in \code{group.by} which is 
...
getDiffExpDESeq2 <- function(
  ...
  if (!requireNamespace('DESeq2')) {
    stop(
      "Please install DESeq2 package to find differentially expressed ", 
      "genes with DESeq2 method"
    )
  }
  ...
  if (is.null(covariates)) {
    ...
    if (all(covariates %in% colnames(metadata))) {
      ...
      colnames(colData) <- c(group.by, covariates)
      deseq2.formula <- stats::as.formula(paste0(
        "~",
        group.by,
        " + ",
