...
transferFeatureData <- function(
  ...
  if (is.null(new_feature_name)) {
    new_feature_name <- paste(
      vrMainFeatureType(object, assay = from)$Feature,
      "pseudo",
      sep = "_"
    )
  }
...
getSpotsFromCells <- function(
  ...
  aggregate_transfer_data <- stats::aggregate(
    t(as.matrix(transfer_data)),
    list(cell_to_spot_nnid),
    sum
  )
  aggregate_transfer_data <- data.frame(
    barcodes = vrSpatialPoints(to_object)
  ) %>%
    dplyr::right_join(aggregate_transfer_data, by = c("barcodes" = "Group.1"))
...
getROIsFromCells <- function(
  ...
  for (i in seq_len(length(segments_rois))) {
    ...
    cell_to_roi_labelid <- c(
      cell_to_roi_labelid,
      rep(names_segments_rois[i], length(in.list.cells))
    )
  }
...
transferLabelsFromTiles2Cells <- function(
  from_object,
  from_metadata = NULL,
  to_object,
  to_metadata = NULL,
    ...
    if (length(data_features) > 0) {
      ...
      if (length(metadata_features) > 1) {
        ...
        if (inherits(from_metadata, "data.table")) {
          raw_counts <- from_metadata[, get(names(from_metadata)[which(
            colnames(from_metadata) == metadata_features
          )])]
        } else {
        ...
        if (expand) {
          ...
          rownames(raw_counts) <- gsub(
            paste0("^", metadata_features, "_"),
            "",
            rownames(raw_counts)
          )
        } else {
...
getJointPCA <- function(
  ...
  if (length(assay_features) > 0) {
    ...
    if (dims > length(features)) {
      message(
        "Requested more PC dimensions than existing features: dims = length(features) now!"
      )
