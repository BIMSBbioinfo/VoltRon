...
#'
#' @noRd
vrSpatialPlotInteractive <- function(plot_g = NULL, shiny.options = list()) {
  # js for Shiny
  shinyjs::useShinyjs()
...
mod_app_server <- function(id, plot_g = NULL) {
  moduleServer(id, function(input, output, session) {
    ranges <- reactiveValues(
      x = plot_g$coordinates$limits$x,
      y = plot_g$coordinates$limits$y
    )
...
configure_shiny_options <- function(shiny.options) {
  ...
  if (!requireNamespace('rstudioapi')) {
    stop(
      "Please install rstudioapi package to use RStudio for interactive visualization"
    )
  }
...
#' vrSpatialPlotInteractive
#'
#' Interactive Plotting identification of spatially resolved cells, spots, 
#' and ROI on associated images from multiple assays in a VoltRon object.
#'
#' @param zarr.file The zarr file of a VoltRon object
#' @param group.by a grouping label for the spatial entities
#' @param reduction The name of the reduction to visualize an embedding 
#' alongside with the spatial plot.
#' @param shiny.options a list of shiny options (host, port etc.) passed 
#' \code{options} argument of \link{wc$widget}.
#'
#' @noRd
vrSpatialPlotVitessce <- function(
  zarr.file,
  group.by = "Sample",
  reduction = NULL,
  shiny.options = NULL
  ...
  if (!requireNamespace('vitessceR')) {
    stop(
      "Please install vitessceR package for using interactive visualization!: ",
  ...
  spatial_segmentation_layer_value <- list(
    opacity = 1,
    radius = 0,
    visible = TRUE,
    stroked = FALSE
  ...
  if (is.null(reduction)) {
    vc$layout(
      vitessceR::hconcat(spatial, vitessceR::hconcat(cell_sets, spatial_layers))
    )
  } else {
    umap <- vc$add_view(
      dataset,
      vitessceR::Component$SCATTERPLOT,
      mapping = reduction
    )
    vc$layout(
      vitessceR::hconcat(
        spatial,
        vitessceR::vconcat(umap, vitessceR::hconcat(cell_sets, spatial_layers))
      )
    )
  }
  ...
  if (length(shiny.options) == 0) {
    ...
    message(
      "Listening widget from ",
      shiny.options[["host"]],
      ":",
      shiny.options[["port"]]
    )
    base = rstudioapi::translateLocalUrl(
      paste0(shiny.options[["host"]], ":", shiny.options[["port"]]),
      absolute = TRUE
    )
