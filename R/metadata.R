...
subset_sampleMetadata <- function(metadata, samples = NULL, assays = NULL) {
  ...
  if (!is.null(samples)) {
    ...
    if (all(assays %in% rownames(metadata))) {
      ...
      stop(
        "Some assay with the names or types '",
...
mergevrMetadata <- function(x, y) {
  ...
  if (length(object_list) > 2) {
    ...
    mol.metadata <- rbind_metadata(
      methods::slot(obj1, "molecule"),
      methods::slot(obj2, "molecule")
    )
    cell.metadata <- rbind_metadata(
      methods::slot(obj1, "cell"),
      methods::slot(obj2, "cell")
    )
    spot.metadata <- rbind_metadata(
      methods::slot(obj1, "spot"),
      methods::slot(obj2, "spot")
    )
    roi.metadata <- rbind_metadata(
      methods::slot(obj1, "ROI"),
      methods::slot(obj2, "ROI")
    )
    tile.metadata <- rbind_metadata(
      methods::slot(obj1, "tile"),
      methods::slot(obj2, "tile")
    )
    combined.metadata <- methods::new(
      "vrMetadata",
      molecule = mol.metadata,
      cell = cell.metadata,
      spot = spot.metadata,
...
cbind_metadata <- function(...) {
  ...
  if (length(metadata_list) > 2) {
    combined.metadata <- cbind_metadata(metadata_list[[1]], metadata_list[[2]])
    for (i in 3:(length(metadata_list))) {
      combined.metadata <- cbind_metadata(combined.metadata, metadata_list[[i]])
    }
    return(combined.metadata)
  } else {
    flag1 <- FALSE
  ...
  if (inherits(metadata, "data.table")) {
    if (nrow(metadata) > 0) {
      if (!is.null(assays)) {
        ...
        selected_sp <- spatialpoints[spatialpoints %in% metadata$id]
        metadata <- metadata[match(selected_sp, id)]
      } else if (!is.null(features)) {
        selected_features <- features[features %in% colnames(metadata)]
        metadata <- if(length(selected_features) > 1) 
    ...
    if (!is.null(assays)) {
      if ("assay_id" %in% colnames(metadata)) {
        ...
        metadata <- metadata[
          stringr::str_extract(cur_column, "Assay[0-9]+") %in% assays,
        ]
      }
    } else if (!is.null(assaytypes)) {
    ...
    if (nrow(metadata) > 0) {
      if (!is.null(assays)) {
        if ("id" %in% colnames(metadata)) {
          metadata <- metadata[
            stringr::str_extract(metadata$id, "Assay[0-9]+") %in% assays,
          ]
        } else {
          if ("assay_id" %in% colnames(metadata)) {
            metadata <- subset(metadata, subset = assay_id %in% assays)
...
addAssayvrMetadata <- function(
  ...
  if (inherits(metadata, "data.table")) {
    ...
    # get original names
    entityID_nopostfix <- stringr::str_replace(
      vrSpatialPoints(assay),
      pattern = "_Assay[0-9]+",
      ""
    )
    entityID <- stringr::str_replace(
      entityID_nopostfix,
      pattern = "$",
      paste0("_", assay_id)
    )
    ...
    # get metadata id
    if ("id" %in% colnames(metadata)) {
      metadata_id <- stringr::str_replace(
    ...
    # initiate metadata
    if (nrow(data) > 0) {
      assay_metadata <- data.frame(
        id = entityID,
        Count = Matrix::colSums(data),
        row.names = NULL
      )
    } else {
      assay_metadata <- data.frame(id = entityID, row.names = NULL)
    }
    ...
    if (!is.null(metadata)) {
      if (length(setdiff(metadata_id, entityID_nopostfix)) > 0) {
        if (nrow(metadata) != length(entityID_nopostfix)) {
          stop(
            "Some spatial points in the metadata 
  ...
  for (sl in methods::slotNames(object)) {
    ...
    if (sl %in% c("molecule", "tile")) {
      ...
      if ("assay_id" %in% colnames(cur_metadata)) {
        ...
        cur_names <- stringr::str_extract(
          as.vector(cur_metadata$id),
          "Assay[0-9]+"
        )
      } else {
...
updateMetadataAssay <- function(object1, object2) {
  ...
  assaytype <- unlist(lapply(object_list, function(obj) {
    if (inherits(obj, "data.table")) {
      ...
      inherits(obj, c("HDF5DataFrame", "ZarrDataFrame", "DataFrame"))
    ) {
  ...
  for (type in all_types) {
    ...
    if (nrow(new_metadata) > 0) {
      ...
      for (i in seq_len(length(old.samples))) {
        new_metadata$Sample[
          new_metadata$Sample == old.samples[i]
        ] <- new.samples[i]
      }

      # change layers
      for (i in seq_len(nrow(sample_metadata_table))) {
        new_metadata$Layer[grepl(
          paste0(sample_metadata_table$AssayID[i], "$"),
          rownames(new_metadata)
        )] <- sample_metadata_table[
          sample_metadata_table$AssayID[i],
          "NewLayer"
        ]
      }
...
setVRMetadata <- function(
  ...
  if (is.null(metadata)) {
    # set metadata
    vr_metadata <- list(
      molecule = data.table::data.table(),
      cell = data.frame(),
      spot = data.frame(),
      ROI = data.frame(),
...
setVRSampleMetadata <- function(samples) {
  ...
  for (i in seq_len(length(sample_list))) {
    ...
    for (j in seq_len(length(layer_list))) {
      ...
      layer_data <- rbind(
        layer_data,
        cbind(names(assay_list), names(layer_list)[j])
      )
    }
  ...
  sample.metadata <- data.frame(
    sample.metadata,
    row.names = paste0("Assay", seq_len(nrow(sample.metadata)))
  )
