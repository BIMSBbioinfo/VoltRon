...
saveVoltRon <- function(
  ...
  if (length(paths) > 1) {
    if (is.null(output)) {
      stop(
        "There are multiple paths that this VoltRon object is saved to, 
    ...
    if (is.null(output)) {
      message(
        "Object has existing paths and 'output' is not specified, 
    ...
    if (length(format) > 1) {
      message(
        "No paths are found in the object, and no format is chosen, 
              saving as rds only!"
      )
  ...
  if (!isSingleString(output)) {
    stop(
      "'output' must be a single string specifying the path ",
      "to the directory where to save the ",
      class(object),
...
loadVoltRon <- function(dir = "my_se") {
  ...
  if (!isSingleString(dir)) {
    stop(
      "'dir' must be a single string specifying the path ",
...
.write_VoltRon <- function(
  ...
  if (!isSingleString(rds_path) || rds_path == "") {
    stop(
      "'rds_path' must be a a non-empty string ",
  ...
  if (!isSingleString(ondisk_path) || ondisk_path == "") {
    stop(
      "'ondisk_path' must be a a non-empty string ",
      "specifying the path to the HDF5 file ",
      "where to write the assays of the ",
      class(object),
      " object"
    )
  }
  ...
  if (format == "HDF5VoltRon") {
    ...
    object <- write_zarr_samples(
      object,
      assay = assay,
      zarr_path = ondisk_path,
      chunkdim,
      level,
      as.sparse,
      verbose,
...
write_h5_samples <- function(
  ...
  object@metadata <-
    writeHDF5ArrayInMetadata(
      object = Metadata(object, type = "all"),
      h5_path,
      name = "metadata",
      chunkdim = chunkdim,
      level = level,
...
#'
#' @noRd
writeHDF5ArrayInMetadata <- function(
  object,
  h5_path,
  name,
  chunkdim,
  ...
  if (!requireNamespace('HDF5DataFrame')) {
    stop(
      "Please install HDF5DataFrame package!: 
         devtools::install_github('BIMSBbioinfo/HDF5DataFrame')"
    )
  }
  if (!requireNamespace('HDF5Array')) {
    stop(
      "Please install HDF5Array package!: 
         BiocManager::install('HDF5Array')"
    )
  }
  if (!requireNamespace('rhdf5')) {
    stop(
      "Please install rhdf5 package!: 
    ...
    if (!inherits(meta.data, c("DataFrame", "HDF5DataFrame")) || replace) {
      if (nrow(meta.data) > 0) {
        ...
        for (i in seq_len(ncol(meta.data))) {
          ...
          if (inherits(meta.data, "data.table")) {
            cur_column <- as.array(
              as.vector(
                subset(meta.data, select = colnames(meta.data)[i])
              )[[1]]
            )
          } else {
      ...
      for (i in seq_len(ncol(meta.data))) {
        ...
        if (!h5Dexists(h5_path, column_name)) {
          if (inherits(meta.data, "data.table")) {
            cur_column <- as.array(
              as.vector(subset(meta.data, select = colnames(meta.data)[i]))[[1]]
            )
          } else {
            cur_column <- as.array(meta.data[, i])
          }
          ...
          new_column <- HDF5Array::writeHDF5Array(
            cur_column,
            h5_path,
            name = column_name,
            chunkdim = chunkdim,
          ...
          new_column <-
            HDF5DataFrame::HDF5ColumnVector(
              DelayedArray::path(new_column),
              name = paste0(name, "/", sn),
...
#'
#' @noRd
writeHDF5ArrayInVrData <- function(
  object,
  h5_path,
  name,
  chunkdim,
...
#'
#' @noRd
writeHDF5ArrayInImage <- function(
  object,
  h5_path,
  name,
  chunkdim,
...
.writeHDF5 <- function(
  ...
  if (feature.vs.obs.engine == "BPCells") {
    if (!requireNamespace('BPCells')) {
      stop(
        "Please install BPCells package!: 
    ...
    if (as.sparse) {
      object <- HDF5Array::writeTENxMatrix(
        object,
        filepath = h5_path,
        group = name,
        level = level,
      ...
      object <- HDF5Array::writeHDF5Array(
        object,
        filepath = h5_path,
        name = name,
        chunkdim = chunkdim,
    ...
    stop(
      "Unrecognized on-disk backed approach, please use
...
write_zarr_samples <- function(
  ...
  for (assy in assay_names) {
    ...
    # get data and write
    assay_object <- writeZarrArrayInVrData(
      object = assay_object,
      zarr_path,
      name = assy,
      chunkdim = chunkdim,
    ...
    # get image data and write
    assay_object <- writeZarrArrayInImage(
      object = assay_object,
      zarr_path,
      name = assy,
      chunkdim = chunkdim,
...
#'
#' @noRd
writeZarrArrayInMetadata <- function(
  object,
  zarr_path,
  name,
  chunkdim,
  ...
  if (!requireNamespace('ZarrDataFrame')) {
    stop(
      "Please install ZarrDataFrame package!: 
...
#'
#' @noRd
writeZarrArrayInVrData <- function(
  object,
  zarr_path,
  name,
  chunkdim,
  ...
  if (
    ...
    for (feat in feature_types) {
      ...
      if (!inherits(a, "DelayedArray") || replace) {
        ...
        # Rarr::ZarrArray doesnt have rownames or colnames
        rownames(a) <- vrFeatures(object)
        colnames(a) <- vrSpatialPoints(object)
...
#'
#' @noRd
writeZarrArrayInImage <- function(
  object,
  zarr_path,
  name,
  chunkdim,
...
.get_unique_links <- function(object, assay = NULL) {
  ...
  all_links <- c(
    all_links,
    .get_unique_metadata_links(Metadata(object, type = "all"))
  )
...
.get_unique_metadata_links <- function(object) {
  ...
  for (sn in slot_names) {
    ...
    path_list <- c(
      path_list,
      ifelse(is(cur_path, "try-error"), "try-error", cur_path)
    )
  }
  path_list
}
...
.get_unique_data_links <- function(object) {
  ...
  if (
    ...
    for (feat in feature_types) {
      cur_path <- try(
        getPath(vrData(object, feat_type = feat, norm = FALSE)),
        silent = TRUE
      )
      all_links <- c(
        all_links,
      ...
      cur_path <- try(
        getPath(vrData(object, feat_type = feat, norm = TRUE)),
        silent = TRUE
      )
      all_links <- c(
        all_links,
...
getPath <- function(object) {
  if (inherits(object, "DelayedArray")) {
    ...
    inherits(object, c("DataFrame", "HDF5DataFrame", "ZarrDataFrame"))
  ) {
...
.shorten_metadata_links <- function(object) {
  ...
  for (sn in slot_names) {
    ...
    if (nrow(meta.data) > 0) {
      for (i in seq_len(ncol(meta.data))) {
        meta.data[[colnames(meta.data)[i]]] <-
          .modify_seeds(meta.data[[colnames(meta.data)[i]]], function(x) {
...
.shorten_vrassayv2_links <- function(object) {
  ...
  for (feat in feature_types) {
    object@data[[feat]] <-
      .modify_seeds(object@data[[feat]], .shorten_assay_links_data)
    object@data[[paste0(feat, "_norm")]] <-
      .modify_seeds(
        object@data[[paste0(feat, "_norm")]],
...
.shorten_assay_links_data <- function(x) {
  if (inherits(x, "IterableMatrix")) {
    ...
    # only update if path exists
    if (length(path.slot) > 0) {
...
.restore_absolute_vrassay_links <- function(object, dir) {
  # get data
  object@rawdata <- .modify_seeds(object@rawdata, \(x) {
    .restore_absolute_links(x, dir)
  })
  object@normdata <- .modify_seeds(object@normdata, \(x) {
    .restore_absolute_links(x, dir)
  })
...
.restore_absolute_vrassayv2_links <- function(object, dir) {
  ...
  feature_types <- vrFeatureTypeNames(object)
  for (feat in feature_types) {
    object@data[[feat]] <- .modify_seeds(object@data[[feat]], \(x) {
      .restore_absolute_links(x, dir)
    })
    object@data[[paste0(feat, "_norm")]] <- .modify_seeds(
      object@data[[paste0(feat, "_norm")]],
      \(x) .restore_absolute_links(x, dir)
    )
  }
...
.restore_absolute_links_delayedarray <- function(x, dir) {
  ...
  # check if path exists, otherwise continue
  if (length(path.slot) > 0) {
    file_path <- slot(x, name = path.slot)
    ...
    # update path
    if (!file.exists(file_path)) {
      stop("Object points to a file ", "that does not exist: ", file_path)
    }
    slot(x, name = path.slot) <- file_path_as_absolute(file_path)
    msg <- validate_absolute_path(
      slot(x, name = path.slot),
      paste0("path slot of Object")
    )
...
setMethod(
  ".restore_absolute_links",
  signature = "ANY",
  .restore_absolute_links_delayedarray
)
...
.restore_absolute_links_iterablematrix <- function(x, dir) {
  ...
  msg <- validate_absolute_path(
    getIterableMatrixPath(x),
    paste0("'filepath' slot of Object")
  )
...
setMethod(
  ...
  .restore_absolute_links_iterablematrix
)
...
.restore_absolute_links_old <- function(x, dir) {
  ...
  if (
    !is(x, c("Array")) &&
      !is(x, c("IterableMatrix")) &&
      !is(x, c("HDF5DataFrame")) &&
      !is(x, c("ZarrDataFrame")) &&
      !is(x, "HDF5ColumnSeed") &&
      !is(x, "ZarrColumnSeed")
  ) {
  ...
  ## not exist.
  if (!file.exists(file_path)) {
    stop("Object points to an HDF5 file ", "that does not exist: ", file_path)
  }
  if (inherits(x, "DelayedArray") || "filepath" %in% slotNames(x)) {
...
create_dir <- function(dir) {
  if (file.exists(dir)) {
    stop("\"", dir, "\" already exists and is a file, ", "not a directory")
  }
