...
setMethod("vrSpatialPoints", "vrSample", function(object) {
  do.call(
    "c",
    lapply(object@layer, function(lay) {
      vrSpatialPoints(lay)
    })
  )
})
...
changeAssayNamesvrSample <- function(object, sample.metadata = NULL) {
  ...
  if (!"NewAssayNames" %in% colnames(sample.metadata)) {
    stop(
      "Please provide a sample.metadata with NewAssayNames column which includes the new assay names"
    )
  }
  ...
  for (lyr in layer_names) {
    object[[lyr]] <- changeAssayNames(
      object[[lyr]],
      sample.metadata = sample.metadata[sample.metadata$Layer == lyr, ]
    )
  }
...
subsetvrLayer <- function(
  ...
  if (!is.null(assays)) {
    ...
    if (
      ...
      if (igraph::vcount(object@connectivity) > 0) {
        ...
        object@connectivity <- subset.Connectivity(
          object@connectivity,
          spatialpoints
        )
      }
  ...
  # remove NULL assays
  object@assay <- object@assay[which(vapply(
    object@assay,
...
setMethod("vrSpatialPoints", "vrLayer", function(object) {
  do.call(
    "c",
    lapply(object@assay, function(assy) {
...
getConnectedSpatialPoints <- function(object, spatialpoints = NULL) {
  if (igraph::vcount(object@connectivity) > 0) {
    spatialpoints <- intersect(
      spatialpoints,
      igraph::V(object@connectivity)$name
    )
    return(names(unlist(igraph::neighborhood(
      object@connectivity,
      nodes = spatialpoints
    ))))
  } else {
...
changeAssayNamesvrLayer <- function(object, sample.metadata = NULL) {
    ...
    if (igraph::vcount(object@connectivity) > 0) {
      ...
      for (i in seq_len(length(old_assay_names))) {
        if (old_assay_names[i] != new_assay_names[i]) {
          ...
          cur_spatialpoints[ind] <- gsub(
            paste0(old_assay_names[i], "$"),
            new_assay_names[i],
            spatialpoints[ind]
          )
        }
  ...
  for (assy in assay_names) {
    vrAssayNames(object[[assy]]) <- rownames(sample.metadata[
      sample.metadata$Assay == assy,
    ])
  }
