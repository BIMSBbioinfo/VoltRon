...
Let us first analyze the Xenium data in single cell level and also visualize molecular data.
...
We now register the H&E image and annotations to the DAPI image of Xenium section using the **registerSpatialData** function. See [Spatial Data Alignment](registration.html) tutorial for more information. In their original 

```{r eval = FALSE, class.source="watch-out"}
vr2_subset <- modulateImage(vr2_subset, brightness = 300, channel = "DAPI")
xen_reg <- registerSpatialData(object_list = list(vr2_subset, imgdata))
...
```{r eval = FALSE, class.source="watch-out"}
vrImages(vr2_merged[["Assay8"]], name = "main", channel = "H&E") <- vrImages(imgdata_reg, assay = "Assay1", name = "image_1_reg")
```
...
```{r eval = FALSE, class.source="watch-out"}
vr2_merged <- addAssay(vr2_merged,
...
Assays: Xenium(Main) Xenium_mol ROIAnnotation 
```
...
vrMainAssay(vr2_merged) <- "ROIAnnotation"
vr2_merged$Region <- vrSpatialPoints(vr2_merged)

# set the spatial coordinate system of ROI Annotations assay
vrMainImage(vr2_merged[["Assay17"]]) <- "image_1_reg"

# transfer ROI annotations to molecules
vr2_merged <- transferData(object = vr2_merged, from = "Assay17", to = "Assay8", features = "Region")

# Metadata of molecules
Metadata(vr2_merged, assay = "Xenium_mol")
```
