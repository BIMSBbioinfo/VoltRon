...
```{r eval = FALSE, class.source="watch-out"}
# get embed data
embed_data <- read.csv("data/BreastTile/img_128.csv")
rownames(embed_data) <- embed_data$tile_id
embed_data <- embed_data[,-1]
embed_data <- as.matrix(embed_data)
# write.csv(embed_data, file = "data/BreastTile/img_128_processed.csv", quote = FALSE, row.names = TRUE)
# embed_data <- read.csv("data/BreastTile/img_128_processed.csv", row.names = 1)
