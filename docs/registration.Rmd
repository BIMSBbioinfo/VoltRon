...
We can then use **registerSpatialData** function as before and select "BSpline (SimpleITK)" option to conduct non-rigid alignment. 
...
A method to clustering Visium spots is using estimated cell type abundances 
per each spot as features for the 3D clustering task. Thus, we first deconvolute
spots into cells using a single cell reference. 
