...
SRBlock <- merge(merge_list[[1]], merge_list[-1], samples = "10XBlock")
SRBlock
```
```
10XBlock: 
...
The function **transferData** detects the types of both the source (from) and the destination (to) assays and determines the how the data should be transfered. In this case, the labels or counts data of each cell in the source Xenium assay will be aggregated into spots in a newly create pseudo Visium assay. The new assay with aggregated counts will be named "Visium_pseudo"

```{r class.source="watch-out", eval = FALSE}
10XBlock <- transferData(10XBlock, from = "Assay1", to = "Assay2")
```

We can now visualize both the original and aggregated counts of a gene, say TACSTD2, to validate the correlation of gene signatures across adjacent tissue secions, and to validate the accuracy of the automated image alignment.

```{r class.source="watch-out", eval = FALSE}
g1 <- vrSpatialFeaturePlot(SRBlock_new, assay = "Visium", features = "TACSTD2", 
                           crop = TRUE, norm = FALSE)
g2 <- vrSpatialFeaturePlot(SRBlock_new, assay = "Visium_pseudo", features = "TACSTD2", 
                           crop = TRUE, norm = FALSE)
g1 + g2
```

If the features argument is specified, and if its a single metadata feature with, e.g. cell types, then the each spot at the new pseudo Visium will be collection of estimated cell type abundances. 

```{r class.source="watch-out", eval = FALSE}
10XBlock <- transferData(10XBlock, from = "Assay1", to = "Assay2", features = "Cluster", 
                         new_assay_name = "Visium_pseudo_Clusters")
vrSpatialFeaturePlot(10XBlock, assay = "Visium_pseudo_Clusters", features = c("3","4"), 
                     crop = TRUE)
```

VoltRon supports multiple assays within in layer. Now, Section 2 includes three spot-type assays: 

* the original Visium spot assay, 
* a pseudo Visium assay with aggregated Xenium raw counts, 
* and estimated Xenium cell type abundances (found by aggregating cell type counts).
...
Assay5 Visium_pseudo_Clusters Section2 10XBlock
```
