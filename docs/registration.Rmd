...
More information on the spatial datasets and the study can be also be found on the [DBITplus manuscript](https://www.nature.com/articles/s41592-025-02948-0). You can also download DBIT-Seq and CODEX datasets from: 
...
In cases where at least of the spatial omics datasets are missing their images, we can use the spatial
...
We first import the DBIT-Seq data, and we also subset based on transcript counts
by spot to 
...
can only be performed manually. Thus we select landmark points and perform an affine or homograph transformation.
...
# transfer images
vrImages(vr_all[["Assay2"]], name = "main_reg", channel = "DAPI") <- 
...
experiment to the DBIT-Seq spots. If not features are defined, all features 
(in this case the mIF markers) will be transfered to DBIT spots as additional 
feature sets.
...
We can visualize the transfered protein expression (i.e. image intensity of the
