...
* **# of Features** option specifies the maximum number of features spotted within each image which will later be used to match to the other image.
...
library(rjson)
params <- rjson::fromJSON("scalefactors_json.json")
...
To use B-spline based fine alignment, we require to install the SimpleITK package as below. 
...
In cases where at least one of the spatial omics datasets is missing their images, we can use the spatial
