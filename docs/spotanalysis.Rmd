...
In addition, we flip the coordinates of the images and and then resize these images. 
...
Lets visualize some protein features on tissue sections. We can either use the DAPI images as a background, or select either "black" or "white" backgrounds (e.g. **background = "white"**)
...
Next, we build neighborhood graphs using the k-nearest neighbor graphs of similarity between dimensionally reduced gene expression profiles. Using this graph, we can conduct a clustering of cells using leiden's method. 
...
Now we can visualize these clusters and perhaps check for inter-condition effects that may reside across the data. 
...
<img width="100%" height="100%" src="images/cellspot_violinclusters.png" class="center">
