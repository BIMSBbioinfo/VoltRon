...
Later, we can move on to normalizing the counts to correct for count depth of cells by **(i)** dividing counts with total counts in each cell, **(ii)** multiply with some constant (default: 10000), and followed by **(iii)** log transformation of the counts.  
...
levels(clusters) <- c("DCIS_1", "DCIS_2_CEACAM6+", "DCIS_3_TACSTD2+", "Macrophages",
                      ...
                      "IT_2", "IT_3_Proliferative", "MyelomaCells", "MastCells", "BCells", 
                      "IT_4_LILRA4+", "PLD4+_LILRA4+_CD4+_Cells")
...
VoltRon also provides support for imaging based proteomics assays as well. In this next use case, we analyze cells characterized by **multi-epitope ligand cartography (MELC)** with a panel of 44 parameters. We use the already segmented cells on which expression of **43 protein features** (excluding DAPI) were mapped to these cells. 
...
The first control case have three fields of views (FOVs). By subsetting on the sample of the first control case, we can visualize only these three sections, and visualize multiple protein markers of interest. 

```{r eval = FALSE, class.source="watch-out", fig.align='center'}
vr_subset <- subset(vr_merged, samples = "control_case_1")
vrSpatialFeaturePlot(vr_subset, features = c("CollagenIV", "CD11b"), alpha = 1, 
                     pt.size = 0.7)
```
