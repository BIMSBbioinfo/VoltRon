...
In this use case, we analyse readouts of the **Xenium In Situ** platform. Two tissue sections are derived from a single formalin-fixed, paraffin-embedded (FFPE) breast cancer tissue block. Two replicate of 5 $\mu$m sections were taken for the Xenium replicates. More information on the spatial datasets and the study can be also be found on the [BioArxiv preprint](https://www.biorxiv.org/content/10.1101/2022.10.06.510405v1).
...
Before moving on to the downstream analysis of the imaging-based data, we can inspect both Xenium images. We use the **vrImages** function to call and visualize reference images of all VoltRon objects. Observe that the DAPI image of the second Xenium replicate is not bright enough.  
...
With **vrSpatialPlot**, we can visualize subcellular components as well as cells. We first visualize mRNAs of ACTA2, a marker for smooth muscle cell actin, and TCF7, an early exhausted t cell marker. We can interactively select a subset of the tissue section and visualize the localization of these transcripts.

```{r eval = FALSE, class.source="watch-out"}
Xen_R1_subsetinfo <- subset(Xen_R1, interactive = TRUE)
Xen_R1_subset <- Xen_R1_subsetinfo$subsets[[1]]
vrSpatialPlot(Xen_R1_subset, transcripts = c("ACTA2", "TCF7"), pt.size = 0.2)
```
...
We can also visualize cell counts on spatially-aware data. The behaviour of **vrSpatialFeaturePlot** (and most plotting functions) depend on the number of assays associated with the assay type (e.g. Xenium). Here, we have to assays, and we visualize two features, hence the resulting plot would include four figures (two features for both assays). 
...
Some number of cells in both Xenium images might have extremely low molecule counts. Although cells are detected at these locations, the low total counts of cells would make it challenging for phenotyping and clustering these cells. Hence, we remove such cells from the VoltRon objects.

```{r eval = FALSE, class.source="watch-out"}
Xen_data <- subset(Xen_data, Count > 5)
```

Later, we can move on to normalizing the counts to correct for count depth of cells by dividing counts with total counts in each cell, multiply with some constant (default: 10000), and followed by log transformation of the counts.  
...
```{r eval = FALSE, class.source="watch-out", fig.align='center'}
vrEmbeddingFeaturePlot(Xen_data, features = c("LRRC15", "TCF7"), embedding = "umap", pt.size = 0.4)
```
...
Next, we build neighborhood graphs with the **shared nearest neighbor (SNN) graph** of cells which are constructed from dimensionally reduced gene expression profiles. The function **getProfileNeighbors** also has an option of building **k-nearest neighbors (kNN)** graphs. We can later conduct a clustering of cells using the **leiden's method** from the igraph package, which is utilized with the **getClusters** function.
...
Now we can label each cell with the associated clustering index and take a look at the clustering accuracy on the embedding space.

```{r eval = FALSE, class.source="watch-out", fig.align='center'}
vrEmbeddingPlot(Xen_data, group.by = "Clusters", embedding = "umap", pt.size = 0.4)
```
...
We can also visualize these clusters on a spatial context. 
...
We can annotate each of these clusters according to their positive markers across 313 features. One can use the **FindAllMarkers** from the [Seurat](https://satijalab.org/seurat/) package to pinpoint these markers by first utilizing the **as.Seurat** function on the Xenium assays of the VoltRon object. For more information on conversion to other packages, please visit the [Converting VoltRon Objects](conversion.html).
...
levels(clusters) <- c("DCIS_1", "DCIS_2_CEACAM6+", "DCIS_3_TACSTD2+", "Macrophages",
                      "Adipocytes", "StromalCells", "IT_1", "CD8_TCells", "CD4_TCells",
                      "EndothelialCells", "ACT2_myoepithelial", "KRT15_myoepithelial",
                      "IT_2", "IT_3_Proliferative", "MyelomaCells", "MastCells", "BCells", "IT_4_LILRA4+",
...
```{r eval = FALSE, class.source="watch-out", fig.align='center'}
vrSpatialPlot(Xen_data, group.by = "CellType", pt.size = 0.13, background = "black", legend.loc = "top")
```
...
The first control case have three fields of views (FOVs). By subsetting on the sample of the first control case, we can visualize only these three sections, and visualize multiple protein markers. 
...
We can utilize dimensional reduction of the available protein markers using the getPCA and getUMAP functions, but now with lower numbers of principal components which are enough to capture information marker list of 44 proteins.
...
We use the **vrNeighbourhoodEnrichment** function to detect cell type pairs that co occur within each other neighborhoods. First, we establish spatial neighborhood graphs that determine the neighbors of each cell on tissue sections. **Delaunay tesselations** or graphs are commonly used to determine neighbors of spatial entities. The function **getSpatialNeighbors** builds a delaunay graph of all assays of a certain type and detects neighbors of cells in a VoltRon object.
...
Here, the number of each cell in a pair in each section is reported to assess the impact of the results of the test (i.e. low number of abundance in one cell type may indicate low impact).
