...
The default (or main) assay of the VoltRon object is typically shown when printed (next to the assay name says "main" in parentheses)
...
The assay type can be provided with the **assay** argument to get type specific assay IDs
...
Each assay a VoltRon object may incorporate indefinite number of coordinate systems. One can look for these coordinate systems using the **vrSpatialNames** function, and select one of systems to call coordinates (see [vrSpatialNames](#image))
...
The arguments of the **vrSegments** functions are identical to vrCoordinates and return a list of polygon corners associated with the coordinate system of the coordinates. 

```{r eval = FALSE, class.source="watch-out"}
vrSegments(xenium_data)
vrSegments(xenium_data, assay = "Xenium")
vrSegments(xenium_data, assay = "Assay1")
vrSegments(xenium_data, spatial = "main")
vrSegments(xenium_data, spatial = "main", reg = TRUE)
```
...
If you want to see the list of all spatial systems associated with all assays of the main assay type, you can call the **vrMainSpatial** function.
...
```{r echo = FALSE, class.source="watch-out", fig.align='center', fig.height=5, fig.width=6, out.width="60%"}
magick::image_ggplot(vrImages(melc_data, spatial = "MELC", channel = "DAPI"))
```
...
If also want to select a channel from the same coordinate system, you can set background argument as a vector of 2 where first is the name of the image (coordinate system) and the other would be channel name. You can get the name of these channels from [vrImageChannelNames](#image).
...
customized density graphs of observations. We use the **n.tile** argument for that. Given the 
