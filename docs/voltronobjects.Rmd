...
In a VoltRon object, each image has a unique ID which is also associated with a coordinate system. The names of these images (or systems) can be called using the **vrImageNames** function.
...
```{r eval = FALSE, class.source="watch-out"}
vrImageNames(visium_data, assay = "Visium")
vrImageNames(visium_data, assay = "Assay1")
```
...
If you wanna see the list of all images/systems associated with all assays of the main assay type, you can call the **vrMainImage** function.
...
```{r eval = FALSE, class.source="watch-out"}
vrMainImage(visium_data, assay = "Visium")
vrMainImage(visium_data, assay = "Assay1")
```
...
Each image object (vrImage) in VoltRon object can also include a indefinite number of channels which you can get (or request) a list of for further use.
...
You can set up the main channel as well as the main image name for later use.
...
```{r eval = FALSE, class.source="watch-out"}
vrSpatialPlot(xenium_data, group.by = "Sample")
```
...
```{r eval = FALSE, class.source="watch-out"}
vrSpatialPlot(xenium_data, group.by = "Sample", plot.segments = TRUE, alpha = 0.6)
```
...
```{r eval = FALSE, class.source="watch-out"}
vrSpatialPlot(xenium_data, group.by = "Sample", plot.segments = TRUE, 
              background = "white")
```

<br>

In cases where there are multiple coordinate systems, you can get the background set to the name of the image (or coordinate system). You can get the name of these images from [vrImageNames](#image).
...
In cases where there are multiple coordinate systems and channel exists in the object, you can set background arguement as a vector of 2, first is the name of the image (coordinate system) and the other would be channel. You can get the name of these images and channels from [vrImageNames](#image) and [vrImageChannelNames](#image).
...
For all variations of **vrSpatialPlot** functions above, you can specifiy the assay names or assay class.
